...
import type { LayoutSectionKey } from "@/types";
...
export function SectionsHubPage() {
  ...
  const getVisibility = (layoutKey?: LayoutSectionKey) => {
    ...
    return layout.find((l) => l.key === layoutKey)?.visible ?? true;
  };

  const getMeta = (card: SectionCard): string | null => {
    if (card.isCustom) {
      const count = state.customSections.length;
      return count === 0 ? "No sections yet" : `${count} section${count > 1 ? "s" : ""}`;
    }
    if (card.layoutKey === "highlights") {
      return `${state.highlights.items.length} videos`;
    }
    if (card.layoutKey === "reels") {
      return `${state.reels.items.length} reels`;
    }
    if (card.layoutKey === "films") {
      return `${state.films.items.length} films`;
    }
    return null;
  };
